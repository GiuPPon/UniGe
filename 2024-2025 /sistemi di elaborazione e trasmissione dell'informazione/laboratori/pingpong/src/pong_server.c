...
/* Ottiene le informazioni di indirizzo per la porta corrente */

if ((gai_rv = getaddrinfo(NULL, port_number_as_str, &gai_hints, &pong_addrinfo)) != 0) {
    fail_errno("UDP Pong failed to get address info");
}

/* Crea il socket UDP */
if ((udp_socket = socket(pong_addrinfo -> ai_family, pong_addrinfo -> ai_socktype, pong_addrinfo -> ai_protocol)) == -1) {
    freeaddrinfo(pong_addrinfo);		// libera la memoria per evitare memory leak
    fail_errno("Failed to make UDP socket");
}

/* Prova a fare il bind alla porta corrente */
if ((bind_rv = bind(udp_socket, pong_addrinfo -> ai_addr, pong_addrinfo -> ai_addrlen)) == 0) {
    /* Se il bind ha successo, salva la porta e restituisce il socket */
    *pong_port = port_number;
    freeaddrinfo(pong_addrinfo);
    return udp_socket;
}

/* Se il bind fallisce, libera le risorse e continua con la prossima porta */
freeaddrinfo(pong_addrinfo);
