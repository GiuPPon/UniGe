...
/*** TO BE DONE 8.0 START ***/
	...
	if(pthread_mutex_lock(&cookie_mutex) != 0)
        fail_errno("get_new_UID error: fail to LOCK");
	...
	if(pthread_mutex_unlock(&cookie_mutex) != 0)
		fail_errno("get_new_UID error: fail to UNLOCK");
...
/*** TO BE DONE 8.0 START ***/
	...
	if(pthread_mutex_lock(&cookie_mutex) != 0)
		fail_errno("keep_track_of_UID error: fail to LOCK");
	
	UserTracker[myUID] = UserTracker[myUID] + 1;
	newcount = UserTracker[myUID];

	if(pthread_mutex_unlock(&cookie_mutex) != 0)
		fail_errno("keep_track_of_UID error: fail to UNLOCK");
...
/*** TO BE DONE 8.0 START ***/
	...
	// gmtime_r variante thread-safe
	if(gmtime_r(&now_t, &now_tm) == NULL)
 		fail_errno("cannot get current time using gmtime_r");
...
/*** TO BE DONE 8.0 START ***/
			...
			file_size = stat_p -> st_size;						// dimensione del file in byte
			file_modification_time = stat_p -> st_mtime;		// tempo di ultima modifica
...
/*** TO BE DONE 8.0 START ***/
			...
			if(stat_p == NULL) {
				...
				if (stat(HTML_404, stat_p) == -1)
					fail_errno("stat error (HTML_404)");
			}
...
/*** TO BE DONE 8.0 START ***/
			...
			if(stat_p==NULL) {
				...
				if (stat(HTML_501, stat_p) == -1)
					fail_errno("stat error (HTML_501)");
			}
...
/*** TO BE DONE 8.0 START ***/
		...
		if(gmtime_r(&file_modification_time, &file_modification_tm) == NULL) 
				fail_errno("Couldn't convert time_t into struct tm");

		strftime(time_as_string, MAX_TIME_STR, "%a, %d %b %Y %T GMT", &file_modification_tm);
		// struct tm = Structure containing a calendar date and time broken down into its components
...
/*** TO BE DONE 8.0 START ***/
	...
	// sendfile - trasferisce dati tra file discriptor
	if(sendfile(client_fd, fd, NULL, file_size) == -1)
		fail_errno("Cannot send fd file on client_fd");
		
	if (close(fd) == -1) 
    	fail_errno("Cannot close file descriptor");
...
/*** TO BE DONE 8.0 START ***/
					...
					// ciclo che controlla tutte le istanze di userID
...
/*** TO BE DONE 8.0 START ***/
				...
				// controlla se il file non è stato modificato dopo la data specificata (If-modified-Since)
				if(difftime(stat_p -> st_mtime, my_timegm(&since_tm)) <= 0) {
					http_method = METHOD_NOT_CHANGED; // file non modificato
				} 
				else
					http_method = http_method - METHOD_CONDITIONAL; 
					// file modificato, rimuove flag condizionale e prepara il server ad inviare il file
