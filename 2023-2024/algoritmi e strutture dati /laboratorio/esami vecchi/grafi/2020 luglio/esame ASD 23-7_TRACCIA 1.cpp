...
bool sonoCollegatiDaUnVertice(const Grafo& g, VerticeID v1, VerticeID v2, VerticeID& vlink) {
    // Controlla se v1 e v2 sono vertici validi
    if (v1 < 0 && (v1 >= g.numVertici) && v2 < 0 || v2 >= g.numVertici) {
        vlink = elementoInesistente;
        return false;
    }

    // Verifica se esiste un vertice vlink che collega v1 a v2
    for (int i = 0; i < g.numVertici; ++i) {
        if (g.matrice[v1][i] > 0 && g.matrice[i][v2] > 0) { // Esiste un arco v1 -> i e un arco i -> v2
            vlink = i;
            return true;
        }
    }

    // Nessun vertice collega v1 a v2
    vlink = elementoInesistente;
    return false;
}
