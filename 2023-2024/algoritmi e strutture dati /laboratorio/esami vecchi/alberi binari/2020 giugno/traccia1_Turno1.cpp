...
void tree::stampa(const Tree& t) {
    ...
    if (t -> sinistro) {
        stampa(t -> sinistro);
        cout << "S " << t -> label << " " << t -> sinistro -> label << endl;
    }
    if (t -> destro) {
        stampa(t -> destro);
        cout << "D " << t -> label << " " << t -> destro -> label << endl;
    }
}
...
int tree::contaNumeroTerminaliIsolati(const Tree& t) {
    int count = 0;
    if (t -> destro == nullptr && t -> sinistro == nullptr) {
        ++count;
    } else {
        contaNumeroTerminaliIsolati(t -> sinistro);
        contaNumeroTerminaliIsolati(t -> destro);
    }
    return count; 
}
