...
// Funzione di supporto per verificare se uno stack contiene un elemento
bool contains(const Stack& s, Elem e) {
    Stack cur = s;
...
Stack stack_union(Stack& s1, Stack& s2) {
    Stack s_union = EMPTYSTACK;  // Stack risultato
    Stack temp = EMPTYSTACK;     // Stack temporaneo per evitare duplicati
 
    while (isEmpty(s1) == false) {
        Elem e = pop(s1);
        if (contains(temp, e) == false) { // Aggiungi solo se non è già presente
            push(e, temp);
        }
    }

    // Inserisci tutti gli elementi di s2 in temp
...
Stack stack_union(Stack& s1, Stack& s2) {
    ...
    while (cur != nullptr) {
        ...
        if (aux -> elem != cur -> elem) {
            push(aux -> elem, s_union);
        } 
        push (cur -> elem, s_union);
    }
