...
bool stack::isEmpty(const Stack& s) {
    return (s == EMPTYSTACK); // ISTRUZIONE AGGIUNTA AI SOLI FINI DELLA COMPILAZIONE
}
...
/****************************************************************/
void stack::make_empty(Stack& s) {  // svuota s
    while (s != EMPTYSTACK) {
        cell *temp = s;
        s = s -> next;
        delete temp;
...
Stack stack_intersection(Stack& s1, Stack& s2) {
    ...
    while (cur != nullptr) {
        aux = s2;
        // Per ogni elemento di s1, viene eseguito un ciclo interno su tutti gli elementi della pila s2 (usando aux per iterare). 
        // Questo confronto avviene per trovare elementi comuni.
        while (aux != nullptr) {
...
Stack stack_intersection(Stack& s1, Stack& s2) {
    ...
    // il while principale scorre s1 un elemento per volta. dentro al while principale c'è un while che
    // scorre tutti gli elementi di s2 e li compara con il cur attuale (ossia UN SOLO ELEMENTO DI S1)
    while (cur != nullptr) {
        aux = s2;
        while (aux != nullptr) {
            if (cur -> elem == aux -> elem) {
